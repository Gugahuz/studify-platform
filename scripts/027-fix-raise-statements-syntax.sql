-- Create the missing functions for exam result calculations (FIXED VERSION)
...
BEGIN
    ...
    RAISE NOTICE 'Updated attempt % with results: % percent (% out of %)', 
...
-- Function to get or create test user (if not exists)
...
BEGIN
    ...
    IF test_user_id IS NULL THEN
        INSERT INTO profiles (
            ...
            'Usuário Teste', 'test@studify.com', '(11) 99999-9999', 'superior'
...
-- Grant permission on the test user function
...
BEGIN
    ...
    IF test_template_id IS NULL THEN
        RAISE NOTICE 'No templates found - run data population scripts first';
        RETURN;
    END IF;
    ...
    ELSE
        RAISE NOTICE 'No questions found for template - check data population';
        DELETE FROM mock_exam_attempts WHERE id = test_attempt_id;
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        -- Clean up on error
        IF test_attempt_id IS NOT NULL THEN
            DELETE FROM mock_exam_responses WHERE attempt_id = test_attempt_id;
            DELETE FROM mock_exam_attempts WHERE id = test_attempt_id;
        END IF;
END $$;
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Some functions may be missing';
    END IF;
END $$;
