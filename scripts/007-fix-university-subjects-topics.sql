-- Purpose: Fixed version of university subjects population script
-- Addresses the ON CONFLICT constraint error

-- First, let's check and create the necessary constraints properly
DO $$
BEGIN
    -- Add unique constraint to flashcard_subjects if it doesn't exist
    IF NOT EXISTS (
    ...
    -- Add unique constraint to flashcard_topics if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- Now insert the university subjects using proper conflict resolution
INSERT INTO public.flashcard_subjects (name, category, description, icon, color) 
VALUES
-- Medicine
...
-- Insert topics using individual INSERT statements to avoid conflicts
DO $$
DECLARE
    medicina_id UUID;
    direito_id UUID;
...
BEGIN
    ...
    -- Insert topics for Administração
...
SELECT '007-fix-university-subjects-topics.sql executed successfully.' AS status;
