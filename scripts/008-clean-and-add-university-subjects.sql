-- Purpose: Clean duplicates and add university subjects
-- Fixes the unique constraint error by removing duplicates first

-- Step 1: Clean duplicate subjects (keep only the first occurrence)
...
-- Step 2: Clean duplicate topics (keep only the first occurrence)
DELETE FROM public.flashcard_topics 
WHERE id NOT IN (
    SELECT MIN(id) 
...
-- Step 3: Now create the unique constraints safely
DO $$
BEGIN
...
-- Step 4: Insert university subjects
...
-- Step 5: Add all university topics
DO $$
DECLARE
    medicina_id UUID;
    direito_id UUID;
...
-- Final verification
SELECT 
    'Script completed successfully. Added 6 university subjects with ' || 
