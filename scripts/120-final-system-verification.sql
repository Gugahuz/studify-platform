-- Verificação completa do sistema após restauração
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mock_exam_templates') THEN
        SELECT COUNT(*) INTO templates_count FROM mock_exam_templates;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mock_exam_questions') THEN
        SELECT COUNT(*) INTO questions_count FROM mock_exam_questions;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mock_exam_attempts') THEN
        SELECT COUNT(*) INTO attempts_count FROM mock_exam_attempts;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mock_exam_responses') THEN
